...
describe Admins::ContentPagesController do
  ...
  describe "synchronization" do
      ...
      context "successful update" do
        before do
          ...
          content_page.update_attributes(origin_id: content_page.id, site_id: PEER_SITE_ID, 
                                         page_name: "old_title")
          put :update ,{ content_page: { parent_content_page_id: "", 
    ...
    describe "POST #move_down" do
      ...
      subject(:content_page) { ContentPage.gen(page_name: "content_page",
                                               sort_order: 1,
    ...
    describe "POST #move_up" do
      let(:content_page_peer_log) { SyncPeerLog.where("sync_object_action_id = ? and sync_object_type_id =? 
                                          ...
                                          SyncObjectAction.swap.id, SyncObjectType.content_page.id,
                                          content_page.origin_id, PEER_SITE_ID).first }
      let(:upper_page_peer_log) { SyncPeerLog.where("sync_object_action_id = ? and sync_object_type_id =? 
                                          and sync_object_id = ? and sync_object_site_id = ?", 
                                          SyncObjectAction.swap.id, SyncObjectType.content_page.id,
                                          upper_page.origin_id, PEER_SITE_ID).first }
      subject(:content_page) { ContentPage.gen(page_name: "content_page",
                                               sort_order: 2,
                                               parent_content_page_id: ContentPage.first.id) }
      ...
      context "successful move up" do
        ...
        it "creates sync peer log for upper page" do
          expect(upper_page_peer_log).not_to be_nil
        end
        
        it "has correct action for upper page" do
          expect(upper_page_peer_log.sync_object_action_id).to eq(SyncObjectAction.swap.id)
        end
        
        it "has correct type for upper page" do
          expect(upper_page_peer_log.sync_object_type_id).to eq(SyncObjectType.content_page.id)
        end
        
        it "sync peer log 'user_site_id' equal 'PEER_SITE_ID' for upper page" do
          expect(upper_page_peer_log.user_site_id).to eq(PEER_SITE_ID)
        end
        
        it "has correct 'user_id' for upper page" do
          expect(upper_page_peer_log.user_site_object_id).to eq(@admin.origin_id)
        end
        
        it "has correct 'object_site_id' for upper page" do
          expect(upper_page_peer_log.sync_object_site_id).to eq(PEER_SITE_ID)
        end
        
        it "has correct 'object_id' for upper page" do
          expect(upper_page_peer_log.sync_object_id).to eq(upper_page.origin_id)
        end
        
        it "creates sync log action parameter for 'content_page_sort_order' for upper page" do
          content_page_sort_order_parameter = SyncLogActionParameter.where(peer_log_id: upper_page_peer_log.id, parameter: "content_page_sort_order")
          expect(content_page_sort_order_parameter[0][:value].to_i).to eq(content_page.sort_order)
        end
        
        it "creates sync log action parameter for 'updated_at' for upper page" do
          upper_page.reload
          updated_at_parameter = SyncLogActionParameter.where(peer_log_id: upper_page_peer_log.id, parameter: "updated_at")
          expect(updated_at_parameter[0][:value]).to eq(upper_page.swap_updated_at.utc.to_s(:db))
        end
